...
//  Вы можете переместить код создания продукта в одно место в программе, что упростит его поддержку.
// + Принцип открытости/закрытости .
//  Вы можете добавлять новые типы продуктов в программу, не нарушая работу существующего клиентского кода.
...
// Все продукты должны соответствовать одному и тому же интерфейсу.
//  Этот интерфейс должен объявлять методы, которые имеют смысл для каждого продукта.

// Добавьте пустой фабричный метод в класс Creator.
//  Тип возвращаемого значения метода должен соответствовать общему интерфейсу продукта.

// В коде создателя найдите все ссылки на конструкторы продукта.
//  Поочередно замените их вызовами фабричного метода, вынеся код создания продукта в фабричный метод.

// Возможно, вам потребуется добавить временный параметр в фабричный метод для управления типом возвращаемого продукта.

// На данном этапе код фабричного метода может выглядеть довольно некрасиво.
//  В нём может содержаться длинное switchвыражение, определяющее,
// какой класс продукта следует создать.Но не волнуйтесь, мы скоро это исправим.

// Теперь создайте набор подклассов-создателей для каждого типа продукта,
//  перечисленного в методе фабрики.Переопределите метод фабрики в подклассах
//  и выделите соответствующие фрагменты кода создания из базового метода.

// Если типов товаров слишком много, и создавать подклассы для всех из них
// не имеет смысла, можно повторно использовать параметр управления из базового класса в подклассах.
